...
static qboolean CompareAtt(char* path, char* name, unsigned musthave, unsigned canthave) {
...
char* Sys_FindFirst(char *path, unsigned musthave, unsigned canhave) {
	...
	while(sceIoDread(fdir, &d) > 0) {
		if(!*findpattern || glob_match(findpattern, d.d_name)) {
			if(CompareAtt(findbase, d.d_name, musthave, canhave)) {
...
void Sys_FindClose (void) {
	if(fdir == 0) {
		Sys_Error("Sys_FindClose without Sys_FindBegin");
		return;
	}
