...
void VID_Init (void) {
	...
	refexport_t Sys_GetRefAPI(refimport_t);
	...
	ri.Com_sprintf = Com_sprintf;
	
	ri.FS_LoadFile = FS_LoadFile;
	ri.FS_FreeFile = FS_FreeFile;
	ri.FS_Gamedir = FS_Gamedir;
	ri.FS_Fopen  = FS_Fopen;
	ri.FS_Fclose = FS_Fclose;
	ri.FS_Fseek  = FS_Fseek;
	ri.FS_Ftell  = FS_Ftell;
	ri.FS_Fread  = FS_Fread;
	...
	if(re.api_version != API_VERSION) {
		Com_Error(ERR_FATAL, "pspgu.prx has incompatible api_version");
	}

	if(!re.Init()) {
		re.Shutdown();
		Com_Error(ERR_FATAL, "pspgu init error");
	}
