...
#define REF_CLEAR_COLOR GU_COLOR_BUFFER_BIT
#define REF_CLEAR_DEPTH GU_DEPTH_BUFFER_BIT
...
typedef struct {
	...
	void	        (*RegisterSky)(const char *name, float rotate, vec3_t axis);
...
typedef struct
{
	...
	void	(*Com_sprintf)(char* s, int s_len, char* str, ...);
	...
	// PSP_FIXME memory waste
	int		(*FS_LoadFile) (char *name, void **buf);
	void	(*FS_FreeFile) (void *buf);
	
	void	(*FS_MakePath)(char* name);

	int		(*FS_Fopen)(const char* name);
	void	(*FS_Fclose)(int fid);
	int		(*FS_Fseek)(int fid, int pos, int func);
	int		(*FS_Ftell)(int fid);
	int		(*FS_Fread)(int fid, void* buffer, int length);
