...
static void SCR_CalcVrect(void) {
	...
	scr_vrect.width = 480*size/100;
	scr_vrect.width &= ~7;

	scr_vrect.height = 272*size/100;
	scr_vrect.height &= ~1;

	scr_vrect.x = (480 - scr_vrect.width)/2;
	scr_vrect.y = (272 - scr_vrect.height)/2;
}
...
void SCR_Sky_f(void) {
	...
	re.RegisterSky(Cmd_Argv(1), rotate, axis);
#endif
}
...
void SCR_Init (void)
{
	...
	scr_conspeed = Cvar_Get ("scr_conspeed", "1.5", 0);
	...
	scr_drawtime = Cvar_Get("scr_drawtime", "0", CVAR_ARCHIVE);
	scr_drawloading = Cvar_Get("scr_drawloading", "0", CVAR_ARCHIVE);
...
void SCR_DrawPause (void) {
	...
	re.DrawPic((480-w)/2, 272/2 + 8, "pause");
}
...
void SCR_DrawLoading (void) {
	...
	re.DrawPic((480-w)/2, (272-h)/2, "loading");
}
...
void SCR_DrawConsole (void) {
	...
	if(cls.state != ca_active || !cl.refresh_prepped) {
		...
		re.DrawFill(0, 272/2, 480, 272/2, 0);
		return;
	}
	*/
...
void SCR_DrawFPS(void) {
	static char fpsstring[8] = { 0 };

	if(cls.key_dest != key_game) {
		return;
	}
	
	if(cls.framecount % 16 == 0) {
		sprintf(fpsstring, "%3d fps", (int)(1.0f / cls.frametime));
	}
	
	int i;
	for(i = 0; i < sizeof(fpsstring) - 1; i++) {
		re.DrawChar(480 - (7 * 8) + (i * 8), 0, fpsstring[i]);
	}
}
...
#if 0
	int msecs = cls.realtime - cls.framecount*10;

	int secs = msecs / 1000;
	int mins = secs / 60;
	secs -= mins * 60;

	sprintf(timestring, "%d:%02d", mins, secs);
#else
	sprintf(timestring, "%d", cls.challenge);
#endif
	
	int i;
	for(i = 0; i < sizeof(timestring) - 1; i++) {
		re.DrawChar(480 - (7 * 8) + (i * 8), 8, timestring[i]);
	}
#endif
}
...
void SCR_UpdateScreen(void) {
	...
	if(scr_draw_loading) {	//  loading plaque over black screen
		re.ClearScreen(REF_CLEAR_COLOR);
		//if(cl.cinematicpalette_active) {
			re.CinematicSetPalette(0);
		...
		re.ClearScreen(REF_CLEAR_DEPTH);
		//if(cl.cinematicpalette_active) {			
			re.CinematicSetPalette(0);
			cl.cinematicpalette_active = false;
		//}
