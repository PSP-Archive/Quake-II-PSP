...
void CL_Pause_f(void) {
	...
	|| !strcmp(Cmd_Argv(1), "yes")
	|| !strcmp(Cmd_Argv(1), "on")) {
...
//printf("TRACE: %s\n", __FUNCTION__);
	...
	Com_Printf("%d.%d.%d.%d:%d\n",
			(unsigned int)adr.ip[0], (unsigned int)adr.ip[1], (unsigned int)adr.ip[2], (unsigned int)adr.ip[3],
			/*(unsigned short)(adr.port & 0xff00 >> 8) | (adr.port & 0x00ff << 8)*/adr.port);
...
//	printf("TRACE: %s\n", __FUNCTION__);
	...
	char path[MAX_QPATH];
	Com_sprintf(path, sizeof(path),"%s/pspconfig.cfg",FS_Gamedir());
	FILE* f = fopen(path, "w");
	if(!f) {
...
void CL_Frame(int msec) {
	...
	if(!cl_timedemo->value) {
		...
		if (extratime < /*1000/cl_maxfps->value*/30)
